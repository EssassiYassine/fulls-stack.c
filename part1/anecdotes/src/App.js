...
const App = () => {
  ...
  const diffrand =(e)=> {
    ...
    let diffrandom  = Math.floor(Math.random() * (anecdotes.length)); 
    while (diffrand===e){
      diffrandom =  Math.floor(Math.random() * (anecdotes.length)); 
    }
    setSelected(diffrandom)
    setcount(votearray[diffrandom])
  }
